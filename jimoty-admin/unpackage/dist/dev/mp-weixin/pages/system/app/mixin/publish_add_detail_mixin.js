  ...
  methods: {
    ...
    formatFormData() {
      ...
      this.keepItems = this.platFormKeys.filter(
        (key) => this.getPlatformChcekbox(key) && (this.formData[key].url || this.formData[key].abm_url || this.formData[key].qrcode_url)
    ...
    async iconUrlDelete(res, key) {
      await this.requestCloudFunction("deleteFile", {
