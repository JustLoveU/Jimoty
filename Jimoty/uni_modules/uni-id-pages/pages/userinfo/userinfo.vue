...
<template>
	<view class="uni-content">
		...
		<uni-list>
			...
			<!-- 设置简介 -->
			<uni-list-item class="item" @click="openDg" title="简介" :rightText="describe" link>
			</uni-list-item>
		...
		<uni-popup ref="dialog2" type="dialog">
			<uni-popup-dialog mode="input" :value="userInfo.describe" @confirm="setDescribe" :inputType="describe"
				title="设置简介" placeholder="请输入您的简介">
			</uni-popup-dialog>
		</uni-popup>
...
<script>
	...
	export default {
		...
		async onLoad(e) {
			...
			const data = await db.collection('uni-id-users').doc(uniCloud.getCurrentUserInfo().uid).field(
				'realname_auth.auth_status as authstatus,describe').get({ getOne: true })
