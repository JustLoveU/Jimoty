	...
	<view>
		<view class="slider-container">
			...
			<movable-area class="sliderBar" :style="{width: (100)+'%'}" :damping="10">
				...
				<movable-view class="slider" direction="all" :x="x" @change="onChange" damping="10" :animation="false">
					<text>{{ score }}</text>
				</movable-view>
			</movable-area>
		</view>
			...
			this.$nextTick(() => {
				el.select(".slider-container").boundingClientRect(function(res) {
					...
					if (that.defaultValue) {
						...
						that.x = ((that.defaultValue - that.minScore) / (that.maxScore - that.minScore)) *
							that.slideBarWidth;
