...
let tokenTime = common_vendor.Zs.getCurrentUserInfo().tokenExpired - Date.now();
if (tokenTime < 0)
  hostUserInfo = {};
const data = {
...
const mutations = {
  ...
  async updateUserInfo(data2 = false) {
    if (data2) {
      ...
      try {
        let res = await usersTable.where("'_id' == $cloudEnv_uid").field(
          "mobile,nickname,username,email,avatar_file,realname_auth.auth_status as realNameStatus,describe,status"
        ).get();
        this.setUserInfo({
          ...res.result.data[0]
        });
